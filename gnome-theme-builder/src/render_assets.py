...
class Assets(object):
    ...
    def check_mark(self, color):
        self.cr.new_sub_path()
        self.cr.set_source_rgb(color[0], color[1], color[2])

        self.cr.move_to(5, 10)
        self.cr.line_to(8, 15)
        self.cr.line_to(16, 5)
        self.cr.stroke()
        self.cr.close_path()

    def rounded_rectangle(self, color, width, height, x, y, radius, alpha=1.0, gradient=False):
        self.cr.new_sub_path()
...
# Make the "checkable" items, e.g. check boxes and radio buttons.
def check_items(border_col, fill_col, mark_col, state, alpha=1.0):
    for scl in [1, 2]:
        if scl == 2:
            ending = '@2.png'
        else:
        ...
        box.rounded_rectangle('shadow', 18, 18, 2, 2, 3)
        box.rounded_rectangle(fill_col, 18, 18, 1, 1, 3)
        box.rounded_rectangle(border_col, 18, 18, 1, 1, 3, alpha=alpha)
        box.rounded_rectangle(fill_col, 16, 16, 2, 2, 2)
        box.save('check-unchecked' + state + ending)
        if state != '':
            # Add a checkmark to the box.
            box.check_mark(mark_col)
            box.save('check-checked' + state + ending)
            # Remove the check mark and put in a hyphen.  Start by redrawing the inner part of the box.
            # Add the hyphen.
            box.rounded_rectangle(fill_col, 16, 16, 2, 2, 2)
            box.line(mark_col, 5, 9, 10, 2)
            box.save('check-mixed' + state + ending)

        # radio
        radio = Assets(20, 20, scl)
        radio.circle('shadow', 11, 11, 9)
        radio.circle(fill_col, 10, 10, 9)
        radio.circle(border_col, 10, 10, 9, alpha=alpha)
        radio.circle(fill_col, 10, 10, 8)
        radio.save('radio-unchecked' + state + ending)
        if state != '':
            radio.circle(mark_col, 10, 10, 3, alpha=alpha)
            radio.save('radio-checked' + state + ending)
            # If we are mixed, redo the outer circle and then put in a line.
            radio.circle(fill_col, 10, 10, 8)
            radio.line(mark_col, 6, 9, 9, 2)
            radio.save('radio-mixed' + state + ending)

        # selectionmode
        selectionmode = Assets(40, 40, scl)
        selectionmode.rounded_rectangle('shadow', 18, 18, 12, 12, 3)
        selectionmode.rounded_rectangle(fill_col, 18, 18, 11, 11, 3)
        selectionmode.rounded_rectangle(border_col, 18, 18, 11, 11, 3, alpha=alpha)
        selectionmode.rounded_rectangle(fill_col, 16, 16, 12, 12, 2)
        selectionmode.save('check-selectionmode-unchecked' + state + ending)
        if state != '':
            selectionmode.rounded_rectangle(border_col, 12, 12, 14, 14, 1, alpha=alpha)
            selectionmode.save('check-selectionmode-checked' + state + ending)


def buttons(color1, color2, color3, state, alpha=1.0):
    button = Assets(20, 20)
    button.rounded_rectangle('shadow', 18, 18, 2, 2, 3)
    button.rounded_rectangle(color1, 18, 18, 1, 1, 3)
    button.rounded_rectangle(color2, 18, 18, 1, 1, 3, alpha=alpha)
    if state == '-active':
        button.rounded_rectangle(color3, 18, 18, 1, 1, 3, gradient=True)
    else:
        button.rounded_rectangle(color1, 16, 16, 2, 2, 2)
        button.rounded_rectangle(color3, 16, 16, 2, 2, 2, gradient=True)
...
def scale_trough(color):
    ...
    trough_h.rounded_rectangle(color, 20, 6, 0, 7, 3)
    trough_h.save('scale-trough-horizontal.png')

    trough_h = Assets(20, 20)
    trough_h.rounded_rectangle(color, 6, 20, 7, 0, 3)
...
def scrollbar_slider(color, state, alpha=1):
    for scl in [1, 2]:
        ...
        slider.rounded_rectangle(color, 30, 10, 0, 5, 5, alpha)
        slider.save('scrollbar-slider-horizontal' + state + ending)

        slider = Assets(20, 30, scl)
        slider.rounded_rectangle(color, 10, 30, 5, 0, 5, alpha)
...
def scrollbar_trough(color):
    for scl in [1, 2]:
        ...
        trough.rounded_rectangle(color, 26, 10, 15, 5, 5, 0.3)
        trough.save('scrollbar-trough-horizontal' + ending)

        trough = Assets(20, 56, scl)
        trough.rounded_rectangle(color, 10, 26, 5, 15, 5, 0.3)
...
def entry(color1, color2, color3, state, alpha=1.0):
    ...
    entry.rounded_rectangle(color2, 18, 18, 1, 1, 3, alpha=alpha)
    entry.rounded_rectangle(color3, 16, 16, 2, 2, 2)
    entry.rounded_rectangle(color3, 16, 16, 2, 2, 2)
...
def toolbar(color1, color2, color3):
    ...
    toolbutton.rounded_rectangle(color1, 18, 18, 1, 1, 3)
    toolbutton.save('toolbutton-toggled.png')

    # Toolbutton hover
    toolbutton = Assets(20, 20)
    toolbutton.rounded_rectangle(color3, 18, 18, 1, 1, 3)
    toolbutton.rounded_rectangle(color2, 16, 16, 2, 2, 2)
...
def progressbar(color1, color2, state=''):
    bar = Assets(10, 10)
    bar.rounded_rectangle(color1, 10, 10, 0, 0, 3)
    bar.save('progressbar-bar' + state + '.png')

    trough = Assets(10, 10)
    trough.rounded_rectangle(color2, 10, 10, 0, 0, 3)
...
if len(sys.argv) == 2:
    ...
    filename = 'schemes/Gnome-Steve.colors'
...
check_fg_inactive = Color(_colors, 'ButtonForegroundInactive'); check_fg_inactive.lighten_color(-.1)
check_border = Color(_colors, 'ButtonBackgroundNormal'); check_border.lighten_color(.30)
check_hover = Color(_colors, 'ButtonForegroundActive')
...
# Unchecked, unhovered.
check_items(check_border.rgb,button_bg.rgb, button_fg.rgb,'')
# Checked, but still unhovered
check_items(check_border.rgb,button_bg.rgb, button_fg.rgb,'-active')
# Hovered.  The only difference here is a different border
check_items(check_hover.rgb,button_bg.rgb, button_fg.rgb,'-hover')

# Disabled
check_items(check_color.insensitive, button_bg.insensitive, check_fg_inactive.rgb, '-insensitive', border_color.insensitive_alpha)
# Enabled, but out of focus
check_items(check_border.rgb,button_bg.rgb, button_fg.rgb, '-backdrop', border_color.inactive_alpha)
# Disabled out of focus
check_items(check_color.insensitive, button_bg.insensitive, check_fg_inactive.rgb,'-backdrop-insensitive', border_color.inactive_insensitive_alpha)
